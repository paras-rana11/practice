...
CONNECTION_BS_240 = None

CONNECTION_TYPE = "TCP/IP"
HOST_ADDRESS = "192.168.1.178"
HOST_PORT = 5000
HOST_CONNECTION_PORT = 5152
REPORT_FILE_PREFIX = 'bs_240_'

# log file path
CASE_FILE = 'C:\\ASTM\\root\\case_file\\complate_case_for_bs_240.json'
...
try:
    ...
    port_mispa_nano_plus = CONNECTION_BS_240.get_connection()
...
async def check_connection():
    global MONITORING_ACTIVE, CONNECTION_BS_240
    try:
        while True:
            MONITORING_ACTIVE = False
            ...
            CONNECTION_BS_240.write(generate_checksum.ENQ.encode())

            byte = CONNECTION_BS_240.read()

            if byte == generate_checksum.ACK.encode():
                CONNECTION_BS_240.write(generate_checksum.EOT.encode())

                logger.info("Machine Still Connected")
                MONITORING_ACTIVE = True
                logger.info("Monitoring Start")
            else:
                CONNECTION_BS_240.close_connection()
                time.sleep(2)
