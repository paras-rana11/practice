...
def addEmployee(request):
    ...
    serialize_data.save()
    return Response({'status':'200','data':serialize_data.data ,'message':"Employee Added"})
...
def updateEmployee(request, id):
    try:
        ...
        s_data.save()
        return Response({'status':'200','data':s_data.data ,'message':"Employee Updated"})
...
def deleteEmployee(request, id):
    try:
        ...
        employee.delete()
        return Response({'status':'200','data':s_data.data ,'message':"Employee Deleted"})
    except Exception as e:
        return Response({'status':'202','errors':str(e) ,'message':"something went wrong"})
